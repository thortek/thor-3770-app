...
<nav class="fixed top-0 z-[100] w-full border-b border-cyan-500/30 bg-gray-900/95 backdrop-blur-sm">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-2">
        ...
        <a href="/" class="flex items-center gap-2">
            ...
            <span class="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-xl font-bold text-transparent">
                THOR DEV
            </span>
        </a>
        ...
        <div class="flex items-center gap-4">
            {#if authState.isAuthenticated}
                ...
                <div class="avatar-dropdown relative">
                    ...
                    >
                        {#if authState.user?.picture}
                            <img
                                ...
                                alt={authState.user?.given_name || 'User'}
                                class="h-10 w-10 rounded-full border-2 border-cyan-500/50 object-cover transition-all duration-300 group-hover:border-cyan-400"
                            />
                        {:else}
                            <!-- Fallback avatar if no picture -->
                            <div class="flex h-10 w-10 items-center justify-center rounded-full border-2 border-cyan-500/50 bg-gradient-to-br from-cyan-500 to-purple-500 text-lg font-bold text-white transition-all duration-300 group-hover:border-cyan-400">
                                {authState.user?.given_name?.[0]?.toUpperCase() || '👤'}
                            </div>
                        {/if}
                    </button>
                    ...
                    <!-- Dropdown Menu -->
                    {#if dropdownOpen}
                        <div class="absolute right-0 z-[110] mt-2 w-56 origin-top-right animate-in slide-in-from-top-2 fade-in">
                            <div class="rounded-lg border border-cyan-500/30 bg-gray-900/95 shadow-xl shadow-cyan-500/10 backdrop-blur-sm">
                                <!-- User Info -->
